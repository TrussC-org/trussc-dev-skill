...
|------|-----------|-------|
| `mouse_move` | `x`, `y`, `button` (opt) | with `button` held, emits a drag; omit for a plain move |
| `mouse_click` | `x`, `y`, `button` | 0 = left, 1 = right. Press+release fire back-to-back |
| `mouse_press` / `mouse_release` | `x`, `y`, `button` | hold/release a button — bracket a drag gesture |
| `mouse_scroll` | `dx`, `dy` | |
| `key_press` / `key_release` | `key` | sokol_app keycode — letters are UPPERCASE (`'A'` = 65) |

**Drag recipe** (EasyCam orbit, slider drags, node dragging):
`mouse_press(x0,y0)` → `mouse_move(x1,y1,button:0)` × N steps → `mouse_release(xN,yN)`.
`mouse_click` alone can NOT drag — its press+release fire in the same instant, so
drag consumers never see an open gesture. (Tools added 2026-06-12; on older cores
